...
<body class="bg-slate-900 min-h-[100dvh] flex items-center justify-center p-4 antialiased selection:bg-brand-500 selection:text-white relative overflow-hidden bg-cover bg-center bg-no-repeat" style="background-image: url('{{ asset('img/wplogin.jpeg') }}?v={{ time() }}');">
    ...
    <div class="w-full max-w-[360px] bg-white/95 backdrop-blur-xl rounded-[1.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-white/20 p-5 sm:p-6 relative z-10 animate-fade-in-up">
        ...
        <div class="text-center mb-4">
            ...
            <h1 class="text-xl font-bold text-slate-900 tracking-tight">Sandi Baru</h1>
            <p class="text-[11px] sm:text-xs text-slate-500 mt-1 leading-relaxed">Silakan masukkan kata sandi baru Anda di bawah ini.</p>
        </div>
        ...
        <form action="{{ route('password.update') }}" method="POST" class="space-y-3">
            ...
            <div class="space-y-1 group">
                <label for="password" class="block text-[10px] sm:text-[11px] font-semibold text-slate-700 transition-colors group-focus-within:text-brand-600">Kata Sandi Baru</label>
                <input type="password" name="password" id="password" placeholder="Minimal 8 karakter" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-xs sm:text-sm rounded-xl px-3 py-2 sm:px-3.5 sm:py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all placeholder:text-slate-400 hover:bg-slate-100" required>
            </div>

            <!-- Confirm Password -->
            <div class="space-y-1 group">
                <label for="password_confirmation" class="block text-[10px] sm:text-[11px] font-semibold text-slate-700 transition-colors group-focus-within:text-brand-600">Ulangi Kata Sandi Baru</label>
                <input type="password" name="password_confirmation" id="password_confirmation" placeholder="Ketik ulang kata sandi" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-xs sm:text-sm rounded-xl px-3 py-2 sm:px-3.5 sm:py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all placeholder:text-slate-400 hover:bg-slate-100" required>
            </div>
