    ...
    <div class="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm flex flex-col sm:flex-row items-center gap-5 relative overflow-hidden">
        ...
        <div class="w-16 h-16 rounded-full bg-emerald-50 border border-emerald-100 text-emerald-600 flex items-center justify-center font-black text-xl shrink-0 shadow-sm overflow-hidden">
            @if($user->profilePhotoUrl())
                ...
                <img src="{{ $user->profilePhotoUrl() }}" alt="Foto profil {{ $user->name }}" class="w-full h-full object-cover" onerror="this.style.display='none'; this.previousElementSibling.classList.remove('hidden')">
            @else
                {{ $initials }}
            @endif
        </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-5 p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                        <div class="relative shrink-0">
                            <div id="profilePhotoPreview" class="w-20 h-20 rounded-full bg-emerald-50 border-2 border-emerald-100 text-emerald-600 flex items-center justify-center font-black text-2xl shadow-sm overflow-hidden">
                                ...
                                <img src="{{ $user->profilePhotoUrl() ?? '' }}" alt="Foto profil" class="w-full h-full object-cover {{ $user->profilePhotoUrl() ? '' : 'hidden' }}" id="profilePhotoPreviewImg" onerror="this.style.display='none'; document.getElementById('profilePhotoPreviewInitials').classList.remove('hidden')">
                            </div>
...
<script>
    function previewProfilePhoto(input) {
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                previewImg.style.display = ''; // Reset inline style set by onerror
    ...
    // Automatically switch tab if there is password/security validation error or tab query parameter is provided
    document.addEventListener('DOMContentLoaded', function() {
        @if($errors->has('current_password') || $errors->has('new_password') || request()->query('tab') === 'security')
            switchTab('security');
        @else
            switchTab('profile');
        @endif
